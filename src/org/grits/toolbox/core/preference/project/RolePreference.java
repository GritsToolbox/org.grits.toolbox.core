...
public class RolePreference extends PreferencePage
{
	...
	protected Control createContents(Composite parent)
	{
		...
		checkboxTableViewer = createRolesTableViewer(container);
		...
		addButton.addSelectionListener(new SelectionListener()
		{
			...
			public void widgetSelected(SelectionEvent e)
			{
				...
				int newRoleCount = 1;
				while(allRoles.contains(ROLE_PREFIX + newRoleCount))
				{
					newRoleCount++;
				}

				String newRole = ROLE_PREFIX + newRoleCount;
				allRoles.add(newRole);

				// for only one role make it default
				if(allRoles.size() == 1)
				{
					defaultRole = newRole;
				}
				...
				tableColumn2.getViewer().editElement(newRole, 1);
			}
		...
		removeButton.addSelectionListener(new SelectionListener()
		{
			...
			private void remove()
			{
				...
				if(!selection.isEmpty())
				{
					...
					if(selectedRole.equals(defaultRole))
					{
						defaultRole = allRoles.isEmpty() ?
								null : allRoles.iterator().next();
					}
					checkboxTableViewer.refresh();
					...
					if(!allRoles.isEmpty())
					{
						selectionIndex = Math.min(selectionIndex, allRoles.size() - 1);
		...
		checkboxTableViewer.setCheckStateProvider(new ICheckStateProvider()
		{
			...
			{
				return element.equals(defaultRole);
			}
		});
		...
		checkboxTableViewer.addCheckStateListener(new ICheckStateListener()
		{
			...
			{
				defaultRole = (String) event.getElement();
			}
		});
	...
	private void loadValues()
	{
		logger.info("Loading preference " +
				ProjectPreferenceStore.Preference.ROLE.getPreferenceName());

		rolePreference  =
				ProjectPreferenceStore.getSingleChoicePreference(
						ProjectPreferenceStore.Preference.ROLE);

		allRoles = new ArrayList<String>();
		allRoles.addAll(rolePreference.getAllValues());
		defaultRole = rolePreference.getDefaultValue();
	}
	...
	{
		Collections.sort(allRoles, String.CASE_INSENSITIVE_ORDER);
		checkboxTableViewer.setInput(allRoles);
		checkboxTableViewer.refresh();
	}

	private CheckboxTableViewer createRolesTableViewer(Composite container)
	{
		Table rolesTable = new Table(container, 
		...
		rolesTable.setLayoutData(tableLayouData);
		...
		tableColumn2.getColumn().setText("Roles");
		...
		tableColumn2.setEditingSupport(new EditingSupport(tableViewer)
		{
			...
			protected void setValue(Object element, Object value)
			{
						...
						&& value instanceof String)
				{
					...
					if(allRoles.contains(element))
						allRoles.set(allRoles.indexOf(element), newValue);
					checkboxTableViewer.refresh();
	...
	{
		Map<String, Boolean> allRolesMap =
				rolePreference.loadDefaultValues(
						ProjectPreferenceStore.Preference.ROLE);
	...
	public boolean performOk() 
	{
		String errorMessage = validateInput();
		setErrorMessage(errorMessage);
		if (errorMessage != null) MessageDialog.openError(Display.getCurrent().getActiveShell(), "Error", "There is an error in \"Role\" preference values. Please fix before saving! Error: " + errorMessage);
	...
	private String validateInput()
	{
		...
		uniqueRoles.addAll(allRoles);
		errorMessage = uniqueRoles.size() == allRoles.size() ?
				errorMessage : "Non-unique Roles";
	...
	{
		rolePreference.setAllValues(new HashSet<String>());
		rolePreference.setDefaultValue(null);
		for(String role : allRoles)
		{
			if(checkboxTableViewer.getChecked(role)
					&& rolePreference.getDefaultValue() == null)
			{
				rolePreference.setDefaultValue(role);
			}
			rolePreference.addValue(role);
		}
		return rolePreference.savePreference();
	}
}