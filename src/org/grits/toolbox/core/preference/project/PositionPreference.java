...
public class PositionPreference extends PreferencePage
{
	...
	public static final String PREFERENCE_PAGE_ID = 
			"org.grits.toolbox.core.preference.project.collaborator.position";
	...
	protected Control createContents(Composite parent)
	{
		...
		checkboxTableViewer = createPositionsTableViewer(container);
		...
		addButton.addSelectionListener(new SelectionListener()
		{
			...
			{
				final String POSITION_PREFIX = "Position ";

				int newPositionCount = 1;
				while(allPositions.contains(POSITION_PREFIX + newPositionCount))
				{
					newPositionCount++;
				}

				String newPosition = POSITION_PREFIX + newPositionCount;
				allPositions.add(newPosition);

				// for only one position make it default
				...
				tableColumn2.getViewer().editElement(newPosition, 1);
			}
		...
		removeButton.addSelectionListener(new SelectionListener()
		{
			...
			private void remove()
			{
				...
				if(!selection.isEmpty())
				{
					...
					if(selectedPosition.equals(defaultPosition))
					{
						defaultPosition = allPositions.isEmpty() ?
								null : allPositions.iterator().next();
					}
					checkboxTableViewer.refresh();
					...
					{
						selectionIndex = Math.min(selectionIndex, allPositions.size() - 1);
		...
		checkboxTableViewer.setCheckStateProvider(new ICheckStateProvider()
		{
			...
			{
				return element.equals(defaultPosition);
			}
		});
		...
		checkboxTableViewer.addCheckStateListener(new ICheckStateListener()
		{
			...
			{
				defaultPosition = (String) event.getElement();
			}
		});
	...
	private void loadValues()
	{
		logger.info("Loading preference " +
				ProjectPreferenceStore.Preference.POSITION.getPreferenceName());

		positionPreference  =
				ProjectPreferenceStore.getSingleChoicePreference(
						ProjectPreferenceStore.Preference.POSITION);

		allPositions = new ArrayList<String>();
		allPositions.addAll(positionPreference.getAllValues());
		defaultPosition = positionPreference.getDefaultValue();
	}
	...
	{
		Collections.sort(allPositions, String.CASE_INSENSITIVE_ORDER);
		checkboxTableViewer.setInput(allPositions);
		checkboxTableViewer.refresh();
	}

	private CheckboxTableViewer createPositionsTableViewer(Composite container)
	{
		Table positionsTable = new Table(container, 
		...
		positionsTable.setLayoutData(tableLayouData);
		...
		tableColumn2.getColumn().setText("Positions");
		...
		tableColumn2.setEditingSupport(new EditingSupport(tableViewer)
		{
			...
			protected void setValue(Object element, Object value)
			{
						...
						&& value instanceof String)
				{
					...
					if(allPositions.contains(element))
						allPositions.set(allPositions.indexOf(element), newValue);
					checkboxTableViewer.refresh();
	...
	{
		Map<String, Boolean> allPositionsMap =
				positionPreference.loadDefaultValues(
						ProjectPreferenceStore.Preference.POSITION);
		...
		for(String position : allPositionsMap.keySet())
		{
			...
			if(allPositionsMap.get(position))
			{
	...
	public boolean performOk() 
	{
		String errorMessage = validateInput();
		setErrorMessage(errorMessage);
		if (errorMessage != null) MessageDialog.openError(Display.getCurrent().getActiveShell(), "Error", "There is an error in \"Position\" preference values. Please fix before saving! Error: " + errorMessage);
	...
	private String validateInput()
	{
		...
		uniquePositions.addAll(allPositions);
		errorMessage = uniquePositions.size() == allPositions.size() ?
				errorMessage : "Non-unique Positions";
	...
	{
		positionPreference.setAllValues(new HashSet<String>());
		positionPreference.setDefaultValue(null);
		for(String position : allPositions)
		{
			if(checkboxTableViewer.getChecked(position)
					&& positionPreference.getDefaultValue() == null)
			{
				positionPreference.setDefaultValue(position);
			}
			positionPreference.addValue(position);
		}
		return positionPreference.savePreference();
	}
}