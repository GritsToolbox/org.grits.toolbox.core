 ...
 *
 */
public class CollaboratorTablePreference extends PreferencePage
{
	...
	private void loadValuesAndSelect()
	{
		logger.info("Loading preference " +
				ProjectPreferenceStore.IntegerPreference.COLLABORATORS_TABLE.getPreferenceName());

		columnPreference =
				ProjectPreferenceStore.getMultiChoiceInteger(
						ProjectPreferenceStore.IntegerPreference.COLLABORATORS_TABLE);

		buttons[0].setSelection(true);
		for(int i = 1; i < CollaboratorTableColumn.COLUMNS.length; i++)
		{
